...
from kmk.keys import Key, KC, make_key,_DefaultKey
...
class Taipo(Module):
    def __init__(self, tap_timeout=200, sticky_timeout=1000):
        ...
        if len(mods) > 0:
            for mod in mods:
                if key.hold_handled:
                    ...
                    keyboard.tap_key(KC.SK(mod))#, tap_time=self.sticky_timeout))
        else:
            if key.hold_handled:
